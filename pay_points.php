  ...
  ?>
      <script>
        inp.value = "Enter 10 digit";
                ...
                ?>
      <script>
        window.location.href = "pay_points_password.php?bu=<?= $bu ?>&id=<?= $id ?>&method=points&price=<?= $result['p_price'] ?>&u_id=<?= $ruser['u_id'] ?>";
      </script>
    <?php
              } else if ($phone != $ruser['phone']) {
    ?><script>
...
<script>
  function changeLanguage(lang) {
    ...
    if (lang === 'th') {
      ...
      content_title_pay3.innerText = 'กรอกเบอร์มือถือ เพื่อใช้คะแนนสะสม';
      ...
      content_title_pay3.innerText = 'Enter mobile number to use accumulated points';
